...
const VoterScreen = () => {
    ...
    const fetchPollData = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5001/api/polls/slug/${slug}`,
        );
        if (res.data.success) {
  ...
  const fetchResults = async (pollId) => {
    try {
      const res = await axios.get(
        `http://localhost:5001/api/polls/public-analytics/${pollId}`,
      );
    ...
    >
      <div style={{ maxWidth: "650px", margin: "0 auto" }}>
        <header style={{ textAlign: "center", marginBottom: "45px" }}>
          ...
          >
            {poll.description ||
              "See public stance and log your voice instantly below."}
          </p>
          ...
          {!hasVoted && (
            ...
            >
              ...
              >
                <div
                  style={{
                    width: `${((currentQuestionIndex + 1) / poll.questions.length) * 100}%`,
                    height: "100%",
        ...
        {!hasVoted ? (
          ...
          >
            ...
            >
              ...
              >
                {analytics
                  .filter((a) => a._id.qIdx === currentQuestionIndex)
            ...
            >
              {currentQuestion.options.map((opt, optIndex) => {
                const result = analytics.find(
                  (a) =>
                    a._id.qIdx === currentQuestionIndex &&
                    String(a._id.optIdx) === String(optIndex),
                );
                const count = result ? result.count : 0;
                const qTotal = analytics
                  .filter((a) => a._id.qIdx === currentQuestionIndex)
                  .reduce((sum, item) => sum + item.count, 0);
                const pct =
                  qTotal === 0 ? 0 : Math.round((count / qTotal) * 100);

                const isSelected =
                  selectedOptions[currentQuestionIndex] === optIndex;

                return (
                  <div
                    key={optIndex}
                    onClick={() =>
                      handleOptionSelect(currentQuestionIndex, optIndex)
                    }
                    style={{
                      position: "relative",
                      padding: "18px 20px",
                      ...
                      background: "#0f172a",
                      border: isSelected
                        ? `2px solid ${poll.accentColor || "#38bdf8"}`
                        : "1px solid #1e293b",
                      cursor: "pointer",
                      overflow: "hidden",
                      ...
                      >
                        <div
                          style={{
                            ...
                            borderRadius: "50%",
                            border: isSelected
                              ? `5px solid ${poll.accentColor || "#38bdf8"}`
                              : "2px solid #475569",
          ...
          <div>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            background: "#0f172a",
                            border: isSelected
                              ? `1px solid ${poll.accentColor || "#38bdf8"}`
                              : "1px solid #1e293b",
                            overflow: "hidden",
                          }}
                        >
            ...
            >
              ✔ Success! All choices have been integrated into the live stream
              metrics.
            </div>
          </div>
        )}
